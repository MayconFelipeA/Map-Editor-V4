...
public OnPlayerEditObject(playerid, playerobject, objectid, response, Float:fX, Float:fY, Float:fZ, Float:fRotX, Float:fRotY, Float:fRotZ) {
    if( !playerobject && objectid == GetPlayerEditObject(playerid)) {
        ...
        static string[64];
        format(string, sizeof(string), "Idx: {FF1C21}%d{D8CD43} - {FF1C21}%d", objectid, GetObjectModel(objectid));
        DeletePlayer3DTextLabel(playerid, objectindex3d[objectid]);
        objectindex3d[objectid] = CreatePlayer3DTextLabel(playerid, string, 0xD8CD43FF, fX, fY, fZ, 900.0);

        switch(response) {
            case EDIT_RESPONSE_FINAL: {
                //CreateText3D(objectid, fX, fY, fZ);
            ...
            case EDIT_RESPONSE_CANCEL: {
                //CreateText3D(objectid, g_ObjectData[objectid-1][OBJECT_DATA_MEMORY_X], g_ObjectData[objectid-1][OBJECT_DATA_MEMORY_Y], g_ObjectData[objectid-1][OBJECT_DATA_MEMORY_Z]);
                ...
                DeletePlayer3DTextLabel(playerid, objectindex3d[objectid]);
                objectindex3d[objectid] = CreatePlayer3DTextLabel(playerid, string, 0xD8CD43FF, g_ObjectData[objectid-1][OBJECT_DATA_MEMORY_X], g_ObjectData[objectid-1][OBJECT_DATA_MEMORY_Y], g_ObjectData[objectid-1][OBJECT_DATA_MEMORY_Z], 900.0);
            }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    switch(dialogid) {
        case DIALOGID_OBJECT_MAIN: {
            ...
            switch(listitem) {
                case LISTITEM_OBJECT_SETMODEL:
                {
                    ShowPlayerDialog(playerid, DIALOGID_SETOBJECT_MODEL, DIALOG_STYLE_INPUT, "Set object model", "Set new modelid to object", "Set", "Cancel");
                    return 1;
                }
        ...
        case DIALOGID_SETOBJECT_MODEL:
        {
            ...
            new newobject = RecreateObject(objectid, .modelid = value);
            if( newobject == INVALID_OBJECT_ID ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: This object could not be re-created!");
            }
            else
            {
                g_PlayerData[playerid][PLAYER_DATA_EDIT_ID] = newobject;
                g_PlayerData[playerid][PLAYER_DATA_EDIT_IDTYPE] = ID_TYPE_OBJECT;
                ShowObjectDialog(playerid, DIALOGID_OBJECT_MAIN);
            }
            return 1;
