...
CopyObject(copy_objectid, bool:copy_attachto = true, modelidupdate = 0) {
    if( !IsValidObject(copy_objectid) ) {
        return INVALID_OBJECT_ID;
    }

    new
               modelid = !modelidupdate ? GetObjectModel(copy_objectid) : modelidupdate,
        Float: x,
        Float: y,
        Float: z,
        Float: rx,
    ...
    if(modelidupdate)
    {
        if( GetModelName(modelid, g_CommentString, sizeof g_CommentString) ) {
            strpack(g_ObjectData[copy_objectid-1][OBJECT_DATA_COMMENT], g_CommentString, sizeof g_CommentString);
        }
    }
...
RecreateObject(copy_objectid, bool:copy_attachto = true, modelid = 0) {
    new paste_objectid = CopyObject(copy_objectid, copy_attachto, modelid);
            ...
            for(new listitem; listitem < MAX_LISTITEMS_OBJECT; listitem ++) {
                switch(listitem) {
                    case LISTITEM_OBJECT_SETMODEL: {
                        strcat(g_DialogInfo, "Set new model object\t \n");
                    }
